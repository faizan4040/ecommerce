...
const initialTasks = [
  { id: 1, name: "Review system logs for any reported errors", created: "23 April, 2024 05:09 PM", due: "30 April, 2024", assigned: "Sean Kemper", avatar: "SK", status: "In-progress", priority: "High" },
  { id: 2, name: "Conduct user testing to identify potential bugs", created: "14 May, 2024 10:51 AM", due: "25 Aug, 2024", assigned: "Victoria Sullivan", avatar: "VS", status: "Pending", priority: "Low" },
...
export default function Todo() {
  ...
  function handleSave() {
    ...
    if (editTask) {
      setTasks(tasks.map(t => t.id === editTask.id ? { ...t, ...form, avatar: initials } : t));
    } else {
      const newTask = { id: Date.now(), created: new Date().toLocaleString("en-GB", { day:"numeric", month:"long", year:"numeric", hour:"2-digit", minute:"2-digit" }), ...form, avatar: initials };
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'DM Sans', sans-serif", padding: "0" }}>
      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .modal { background:#fff;border-radius:20px;padding:36px 32px;width:100%;max-width:480px;box-shadow:0 24px 64px rgba(15,23,42,0.18); }
        .input-field { width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:14px;font-family:inherit;background:#f8fafc;transition:border 0.15s;outline:none; }
        ...
        .confirm-modal { background:#fff;border-radius:16px;padding:32px;max-width:360px;width:100%;text-align:center;box-shadow:0 24px 64px rgba(15,23,42,0.18); }
      `}</style>
      ...
      <div style={{ background: "#fff", borderBottom: "1.5px solid #e2e8f0", padding: "20px 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          ...
          <div style={{ color: "#64748b", fontSize: 14, marginTop: 2 }}>Admin Dashboard · {tasks.length} total tasks</div>
        </div>
        ...
        <div style={{ background: "#fff", borderRadius: 18, border: "1.5px solid #e2e8f0", overflow: "hidden", boxShadow: "0 2px 16px rgba(15,23,42,0.06)" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              ...
              <tbody>
                {paginated.length === 0 ? (
                  <tr><td colSpan={7} style={{ padding: "48px", textAlign: "center", color: "#94a3b8", fontSize: 15 }}>No tasks found.</td></tr>
          ...
          <div style={{ padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1.5px solid #f1f5f9", flexWrap: "wrap", gap: 12 }}>
            <span style={{ color: "#64748b", fontSize: 14 }}>
              Showing <strong style={{ color: "#0f172a" }}>{Math.min(paginated.length, ITEMS_PER_PAGE)}</strong> of <strong style={{ color: "#0f172a" }}>{filtered.length}</strong> tasks
            </span>
        ...
        <div className="modal-overlay" onClick={e => { if(e.target===e.currentTarget) setShowModal(false); }}>
          <div className="modal">
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 28, justifyContent: "flex-end" }}>
              ...
              <button className="create-btn" onClick={handleSave}>{editTask ? "Save Changes" : "Create Task"}</button>
            </div>
          </div>
        </div>
      )}
